...
void PrintIncorrectLines(int k0, int k1, int k2, fstream &fprotok) {
	int K[3] = { k0, k1, k2 };
	string S[3];
	int k;
	for (int i = 0; i < 3; i++) {
		...
		do {
			...
			if (fin.eof()) {
				...
				if (k < n) {
					...
					PrintIncorrectLines(k0, k1, k2, fprotok);
				}
				else if (k > n) {
...
int PrintPoints(float POINTS[N][2], int n, fstream& fprotok) {
...
int main() {
	...
	fin.open("points.txt", ios::in);
	frez.open("result.txt", ios::out);
	fprotok.open("protokol.txt", ios::out);
	...
	if (isErrors) {
		...
		fin.close(); fin.open("points.txt", ios::in);
		n = GetPoints(fin, POINTS, kmax); // заполняет массив POINTS заданными точками и возвращает кол-во точек
		PrintPoints(POINTS, n, fprotok); // выводит все считанные точки в файл протокола
